...
export default async function ProductDetailPage({
  ...
  // 1. Get Global Product Banner from Pages table
  const globalPageResult = await query("SELECT bannerImage FROM pages WHERE slug = 'product-detail' AND isActive = 1 LIMIT 1") as any[];
  const globalBanner = globalPageResult[0]?.bannerImage;

  const bannerImg = product.bannerImage || globalBanner || "https://fohffyjhcwci6coi.public.blob.vercel-storage.com/at%20a%20glance%20bannner.png";
            ...
            <div className="space-y-2">
              <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-3">
                <div className="space-y-2 flex-1">
                  ...
                  <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-primary tracking-tight leading-none max-w-2xl">
                    {product.name}
                  </h1>
              ...
              <div className="max-w-none text-slate-600 font-medium leading-[1.6] border-l-4 border-primary/10 pl-8 py-1 text-xl
                [&_p]:mb-4
                      ...
                      return sortedEntries.map(([key, value]) => (
                        ...
                        >
                          ...
                          {key.toLowerCase().includes("mating part") ? (
                            ...
                            <span className="text-xs font-bold text-slate-600 leading-tight text-right">
                              {(!value || String(value).trim() === "" || String(value).trim() === ".") ? "-" : String(value)}
                            </span>
                          )}
                        </div>
                      ));
