...
export default function NewsSectionEditor({
    ...
    const [images, setImages] = useState<NewsImage[]>(() => {
        const initialImages = content.images || [];
        if (initialImages.length < 6) {
            return [...initialImages, ...Array(6 - initialImages.length).fill({ url: "", title: "", date: "" })];
        }
        return initialImages.slice(0, 6);
    });
    const [backgroundImage, setBackgroundImage] = useState(content.backgroundImage || "");
    const [clientLogos, setClientLogos] = useState<ClientLogo[]>(() => {
        const initialLogos = content.clientLogos || [];
        if (initialLogos.length < 16) {
            return [
                ...initialLogos, 
                ...Array(16 - initialLogos.length).fill(null).map((_, i) => ({ id: `logo-${initialLogos.length + i}`, src: "", alt: "" }))
            ];
        }
        return initialLogos.slice(0, 16);
    });
        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 text-gray-900">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-8 overflow-y-auto space-y-8">
                    ...
                    {/* Collage Images */}
                    <div className="space-y-6">
                        <h3 className="text-lg font-bold text-gray-900">Collage Images (6 total)</h3>
                    ...
                    <div className="space-y-6">
                        <h3 className="text-lg font-bold text-gray-900">Trusted By Logos (16 max for last slide)</h3>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {clientLogos.map((logo, index) => (
                                <div key={index} className="p-3 border border-gray-100 rounded-xl bg-gray-50/50 space-y-3">
                                    <div className="flex items-center justify-between">
                                        <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full uppercase">Logo {index + 1}</span>
                                    </div>
                                    <div className="space-y-2">
                                        {/* Preview */}
                                        ...
                                        <div className="space-y-1">
                                            <input
                                                type="text"
                                                value={logo.src}
                                                onChange={(e) => updateClientLogo(index, "src", e.target.value)}
                                                className="w-full p-2 bg-white border border-gray-200 rounded-lg text-xs"
                                                placeholder="Image URL..."
                                            />
                                            <input
                                                type="text"
                                                value={logo.alt}
                                                onChange={(e) => updateClientLogo(index, "alt", e.target.value)}
                                                className="w-full p-2 bg-white border border-gray-200 rounded-lg text-xs"
