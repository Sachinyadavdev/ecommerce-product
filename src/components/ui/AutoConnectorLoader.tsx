...
function buildBP(cx: number, cy: number) {
  ...
  a(mx, cy - mh / 2, mx + mw, cy - mh / 2, .18, .23); a(mx + mw, cy - mh / 2, mx + mw, cy + mh / 2, .22, .26);
  a(mx + mw, cy + mh / 2, mx, cy + mh / 2, .25, .29); a(mx, cy + mh / 2, mx, cy - mh / 2, .28, .32);
  for (let r = 0; r < 3; r++)for (let c = 0; c < 2; c++) {
...
function drawHousing(ctx: CanvasRenderingContext2D, cx: number, cy: number, fw: number, fh: number, side: "L" | "R", alpha: number, ft: number) {
  ...
  if (s > 0) {
    ctx.globalAlpha = alpha * s * .5; ctx.fillStyle = C_DARK;
    ctx.beginPath();
    ...
    ctx.globalAlpha = alpha * s * .35; ctx.fillStyle = C_MDARK;
    ctx.beginPath();
  ...
  g.addColorStop(0, s > .3 ? C_LITE : "transparent"); g.addColorStop(1, s > .3 ? C_MDARK : "transparent");
  ctx.fillStyle = g; ctx.beginPath(); ctx.roundRect(cx - fw / 2, cy - fh / 2, fw, fh, 4); ctx.fill();
  ctx.globalAlpha = alpha; ctx.strokeStyle = s > .3 ? C_LITE : C_DARK; ctx.lineWidth = s > .3 ? .9 : .65;
  ctx.setLineDash(s > .3 ? [] : [4, 4]); ctx.beginPath(); ctx.roundRect(cx - fw / 2, cy - fh / 2, fw, fh, 4); ctx.stroke(); ctx.setLineDash([]);
  if (s > .4) {
    const rx = side === "L" ? cx - fw / 2 + 5 : cx + fw / 2 - 10;
    ctx.globalAlpha = alpha * s * .8; ctx.fillStyle = "#050505";
    for (let i = 0; i < 3; i++) { ctx.fillRect(rx, cy - 8 + i * 7, 4, 5); ctx.strokeStyle = C_LITE; ctx.lineWidth = .35; ctx.strokeRect(rx, cy - 8 + i * 7, 4, 5); }
  }
  ctx.restore();
}
...
function drawLatch(ctx: CanvasRenderingContext2D, cx: number, cyTop: number, side: "L" | "R", alpha: number, ft: number, lifted: boolean) {
  ...
  ctx.save(); ctx.globalAlpha = alpha;
  ctx.fillStyle = side === "R" && s > .4 ? "#1a66ff" : "transparent"; // Electric Blue tab
  ctx.strokeStyle = side === "R" && s > .4 ? "#4a8bff" : s > .4 ? C_LITE : C_DARK;
  ...
  if (s > .4 && side === "R") {
    ctx.strokeStyle = "rgba(255,255,255,0.4)"; ctx.lineWidth = 1; ctx.globalAlpha = alpha;
...
function drawCavityFace(ctx: CanvasRenderingContext2D, cx: number, cy: number, fw: number, litN: number, alpha: number, ft: number) {
  ...
  ctx.save();
  if (s > .3) {
    ctx.globalAlpha = alpha; ctx.fillStyle = C_DARK; ctx.beginPath();
    ctx.roundRect(fx - 4, fy - 4, 44, 52, 4); // Much wider Face Outside shroud
    ctx.fill(); ctx.strokeStyle = "#44444a"; ctx.lineWidth = .65; ctx.beginPath();
    ctx.roundRect(fx - 4, fy - 4, 44, 52, 4); ctx.stroke();
  }
  for (let r = 0; r < 3; r++)for (let c = 0; c < 2; c++) {
    const i = r * 2 + c, gx = fx + 4 + c * 16, gy = fy + 8 + r * 13; // 6 boxes
    ctx.globalAlpha = alpha * (s > .25 ? 1 : s * 4);
    if (i < litN) { ctx.fillStyle = "#2a88ff"; ctx.shadowColor = "#4aaaff"; ctx.shadowBlur = 10; }
    else { ctx.fillStyle = s > .3 ? "#081222" : "transparent"; ctx.shadowBlur = 0; }
    ctx.strokeStyle = s > .3 ? "#183050" : C_DARK; ctx.lineWidth = .8;
    ctx.setLineDash(s > .3 ? [] : [3, 3]); ctx.beginPath();
    ctx.roundRect(gx, gy - 2, 10, 8, 2); // Blue box
...
function drawPinFace(ctx: CanvasRenderingContext2D, cx: number, cy: number, fw: number, litN: number, alpha: number, ft: number) {
  ...
  ctx.save();
  if (s > .3) { ctx.globalAlpha = alpha; ctx.fillStyle = "#060608"; ctx.beginPath(); ctx.roundRect(px, pfy, 36, 44, 3); ctx.fill(); ctx.strokeStyle = "#1a1a1c"; ctx.lineWidth = .55; ctx.beginPath(); ctx.roundRect(px, pfy, 36, 44, 3); ctx.stroke(); }
  for (let r = 0; r < 3; r++)for (let c = 0; c < 2; c++) {
    const i = r * 2 + c, gx = px + 6 + c * 16, gy = pfy + 8 + r * 13; // Horizontal position
    ctx.globalAlpha = alpha * (s > .25 ? 1 : s * 4);
    ctx.fillStyle = s > .3 ? "#ddaa33" : "transparent"; // Always gold
    if (i < litN) { ctx.shadowColor = "#ffcc44"; ctx.shadowBlur = 10; }
    else { ctx.shadowBlur = 0; }
    ctx.strokeStyle = s > .3 ? "#997722" : "#110000"; ctx.lineWidth = .5;
    ctx.setLineDash(s > .3 ? [] : [2, 3]);
    if (s > .3) {
      // Horizontal blades ("line inside")
      ctx.fillRect(gx, gy, 12, 3); ctx.strokeRect(gx, gy, 12, 3);
      ctx.fillStyle = "rgba(255,255,255,0.15)";
      ctx.fillRect(gx + 12, gy + 1, 6, 1); // Inner wire tail detail
    }
    else { ctx.beginPath(); ctx.moveTo(gx, gy + 1.5); ctx.lineTo(gx + 12, gy + 1.5); ctx.stroke(); }
...
/* ── Phase metadata ── */
const PHASE_LABELS = ["PHASE 01 — BLUEPRINT DRAFT", "PHASE 02 — COMPONENT FORMING", "PHASE 03 — MATING APPROACH", "PHASE 04 — TERMINAL ENGAGE", "PHASE 04 — TERMINAL SECURED", "PHASE 01 — BLUEPRINT DRAFT"];
const TSTATUSES = ["INIT", "FORMING", "APPROACH", "ENGAGE", "LOCKED", "RESET"];
const DOT_STATES = [["active", "", "", ""], ["done", "active", "", ""], ["done", "done", "active", ""], ["done", "done", "done", "active"], ["done", "done", "done", "done"], ["", "", "", ""]];

/* ── Component ── */
export default function AutoConnectorLoader() {
  ...
  const render = useCallback((ts: number) => {
    ...
    if (!cv) { rafRef.current = requestAnimationFrame(render); return; }
    ...
    if (ph === 0) {
      ...
      ctx.save(); ctx.globalAlpha = ez.out3(clamp(pt * 6, 0, 1)) * .22; ctx.strokeStyle = BLUE; ctx.lineWidth = .4;
  ...
  useEffect(() => {
    if (pathname.startsWith("/admin")) {
      ...
      return () => { clearTimeout(t); stopLoader(); };
    }
    else stopLoader();
  }, [pathname, searchParams, startLoader, stopLoader]);

  useEffect(() => {
    const s = () => { if (window.location.pathname.startsWith("/admin")) startLoader(); };
    const e = () => stopLoader();
  ...
  const dotStyle = (st: string): React.CSSProperties => ({
    width: 5, height: 5, borderRadius: "50%", flexShrink: 0, transition: "all .4s ease",
    background: st === "done" ? GREEN : st === "active" ? BLUE : "rgba(22,60,140,.3)",
    boxShadow: st === "done" ? `0 0 5px ${GREEN}80` : st === "active" ? `0 0 6px ${BLUE}90` : "none",
  });
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          {/* Corner brackets */}
          {[
            { top: 28, left: 28, borderTop: `1.5px solid ${BLUE}`, borderLeft: `1.5px solid ${BLUE}` },
            { top: 28, right: 28, borderTop: `1.5px solid ${BLUE}`, borderRight: `1.5px solid ${BLUE}` },
            { bottom: 28, left: 28, borderBottom: `1.5px solid ${BLUE}`, borderLeft: `1.5px solid ${BLUE}` },
            { bottom: 28, right: 28, borderBottom: `1.5px solid ${BLUE}`, borderRight: `1.5px solid ${BLUE}` },
          ].map((s, i) => (
            <motion.div key={i} animate={{ opacity: [.2, .6, .2] }} transition={{ duration: 2.5, repeat: Infinity, delay: i * .6 }}
              style={{ position: "absolute", width: 36, height: 36, ...s as any }} />
          ))}

          {/* Telemetry row */}
          <div style={{ position: "absolute", top: 30, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 28, zIndex: 2 }}>
            {[
              { lbl: "Voltage", val: "24.0V" },
              { lbl: "Pins", val: `${litN}/6` },
              { lbl: "Signal", val: locked ? "OK" : "—", green: locked },
              { lbl: "Status", val: TSTATUSES[phase] || "" },
            ].map((t, i) => (
              <div key={i} style={{ fontSize: 7, letterSpacing: ".18em", color: `rgba(2,169,87,.7)`, fontFamily: "'Courier New',monospace", textAlign: "center", textTransform: "uppercase" }}>
                {t.lbl}
                <div style={{ fontSize: 10, fontWeight: "bold", color: t.green ? GREEN : `rgba(2,169,87,1)`, letterSpacing: ".1em", marginTop: 2, fontFamily: "'Courier New',monospace", textShadow: t.green ? `0 0 8px ${GREEN}` : undefined }}>
                  {t.val}
                </div>
              </div>
            ))}
          ...
          <div style={{ position: "relative", zIndex: 2, display: "flex", flexDirection: "column", alignItems: "center" }}>
            {/* Phase label */}
            <motion.div animate={{ opacity: [.7, 1, .7] }} transition={{ duration: 2, repeat: Infinity }}
              style={{ fontSize: 9, letterSpacing: ".3em", textTransform: "uppercase", color: GREEN, fontFamily: "'Courier New',monospace", marginBottom: 22, textShadow: `0 0 12px ${GREEN}60` }}>
              {PHASE_LABELS[phase]}
            </motion.div>
            ...
            <div style={{ marginTop: 26, display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 18, alignItems: "center" }}>
                {["Blueprint", "Forming", "Assembly", "Locked"].map((lbl, i) => {
                  const st = (DOT_STATES[phase] ?? ["", "", "", ""])[i] ?? "";
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 7.5, letterSpacing: ".16em", textTransform: "uppercase", color: `rgba(2,169,87,.5)`, fontFamily: "'Courier New',monospace" }}>
                      <div style={dotStyle(st)} />{lbl}
                    </div>
                  );
                })}
              </div>
          ...
          {/* Fixed corner labels */}
          <div style={{ position: "absolute", bottom: 30, right: 36, fontSize: 8, letterSpacing: ".35em", textTransform: "uppercase", color: `rgba(2,169,87,.6)`, fontFamily: "'Courier New',monospace", zIndex: 2 }}>
            Besmak India
          </div>
          <div style={{ position: "absolute", bottom: 30, left: 36, fontSize: 7.5, letterSpacing: ".2em", color: `rgba(22,60,140,.7)`, fontFamily: "'Courier New',monospace", zIndex: 2 }}>
            BES-MF6 / REV.C
          </div>
        </motion.div>
      )}
