...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const endElementTop = endElement ? getElementOffset(endElement, 0) : 0;
    
    // Calculate a universal pinEnd based on the last card's true height
    const lastCard = cardsRef.current[cardsRef.current.length - 1];
    const lastCardHeight = lastCard ? lastCard.offsetHeight : 0;
    const totalStackOffset = itemStackDistance * Math.max(0, cardsRef.current.length - 1);
    const stackPinEnd = endElementTop - stackPositionPx - totalStackOffset - lastCardHeight;
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardsRef.current.length; j++) {
          const jTransform = lastTransformsRef.current.get(j);
          const jTranslateY = jTransform ? jTransform.translateY : 0;
  ...
  useLayoutEffect(() => {
    ...
    let initialUpdateTimeout = setTimeout(() => {
      updateCardTransforms();
    }, 100);
