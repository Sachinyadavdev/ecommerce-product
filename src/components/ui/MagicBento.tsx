...
export const DEFAULT_GLOW_COLOR = '0, 167, 88'; // Primary green rgb
...
export const GlobalSpotlight = ({
  ...
  useEffect(() => {
    ...
    spotlight.className = 'fixed w-[800px] h-[800px] rounded-full pointer-events-none z-[200] mix-blend-screen opacity-0 -translate-x-1/2 -translate-y-1/2';
...
export const MagicBentoContainer = ({ children, className = "", enableSpotlight = true, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = DEFAULT_GLOW_COLOR }: any) => {
  const gridRef = useRef<HTMLDivElement>(null);
  const isMobile = useMobileDetection();

  return (
    <>
      <GlobalSpotlight gridRef={gridRef} disableAnimations={isMobile} enabled={enableSpotlight} spotlightRadius={spotlightRadius} glowColor={glowColor} />
...
export const MagicBentoCard = ({
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      ripple.style.background = `radial-gradient(circle, rgba(${glowColor}, 0.3) 0%, rgba(${glowColor}, 0.1) 30%, transparent 70%)`;
  ...
  return (
    <div
      ref={cardRef}
      className={`magic-bento-card ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''} particle-container ${className} group`}
