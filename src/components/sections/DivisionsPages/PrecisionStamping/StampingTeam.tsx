...
const defaultRoles: TeamRole[] = [
  { title: "Design & Engineering", description: "Responsible for developing high-precision tools and process optimization.", icon: "Briefcase" },
  { title: "Operations", description: "The backbone ensuring seamless production and high-efficiency output.", icon: "Settings" },
  { title: "Sales", description: "Focused on client relationships and market demands.", icon: "TrendingUp" },
  { title: "Quality Control", description: "Committed to upholding the highest standards before reaching customers.", icon: "Award" }
];

const iconMap: Record<string, any> = { Users, Award, Briefcase, TrendingUp, Settings };
...
export default function StampingTeam({ content }: StampingTeamProps) {
  ...
  const achievementSub = achievement.includes(' – ') ? achievement.split(' – ')[1] : "";
  ...
  return (
    <section 
      className="py-24 md:py-32 text-white site-content relative overflow-hidden bg-slate-950 bg-cover bg-center bg-fixed"
    ...
    >
      ...
      <div className="container mx-auto px-4 max-w-7xl relative z-10">
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-24 items-center">
          ...
          >
            <div className="bg-white/5 backdrop-blur-3xl p-10 md:p-16 lg:p-20 rounded-[3rem] md:rounded-[4rem] border border-white/10 relative overflow-hidden text-center shadow-[0_0_80px_-15px_rgba(0,0,0,0.5)] group hover:border-white/20 transition-colors duration-500">
              
              {/* Inner Glow Array */}
              <div className="absolute top-0 inset-x-0 h-px bg-linear-to-r from-transparent via-primary/50 to-transparent opacity-50" />
              <div className="absolute bottom-0 inset-x-0 h-1 bg-linear-to-r from-primary/0 via-primary/20 to-primary/0 scale-x-0 group-hover:scale-x-100 transition-transform duration-700 ease-out" />
              ...
              <div className="relative z-10">
                <Users size={56} className="mx-auto mb-10 text-primary opacity-90 group-hover:scale-110 transition-transform duration-500" strokeWidth={1.5} />
                
                <h3 className="text-xl md:text-2xl font-bold text-slate-400 tracking-widest uppercase mb-4">
                  Strength of
                </h3>
                ...
                <h3 className="text-2xl md:text-3xl font-bold text-white tracking-tight">Professionals</h3>
              </div>
            </div>
            ...
            <div className="mt-8 lg:mt-0 lg:absolute lg:-bottom-8 lg:left-1/2 lg:-translate-x-1/2 z-20 flex justify-center w-full">
              ...
              >
                ...
                <div className="text-center md:text-left">
                  <div className="text-lg md:text-xl tracking-tight">{achievementMain}</div>
                  {achievementSub && <div className="text-xs text-slate-500 font-medium uppercase tracking-wider">{achievementSub}</div>}
                </div>
              </motion.div>
            </div>
          </motion.div>
              ...
              {displayRoles.map((role, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className="p-8 rounded-4xl bg-white/2 border border-white/5 hover:border-white/10 transition-all duration-300 group flex flex-col h-full relative overflow-hidden"
                  >
                    ...
                    <div className="w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-primary mb-6 group-hover:bg-primary group-hover:text-white transition-colors duration-300 relative z-10 shrink-0">
                      <Icon size={24} strokeWidth={1.5} />
                    </div>
                    
                    <div className="relative z-10 mt-auto">
                      <h4 className="font-bold text-2xl text-white mb-3 tracking-tight group-hover:text-primary transition-colors">{role.title}</h4>
                      <p className="text-slate-400 font-light text-sm md:text-base leading-relaxed group-hover:text-slate-300 transition-colors">{role.description}</p>
                    </div>
                  </motion.div>
                );
              })}
