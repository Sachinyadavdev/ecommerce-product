...
export function Footer({ settings }: FooterProps) {
  ...
  const verticals = settings?.footer_verticals
    ...
    : [
      ...
      { title: "Engineering Products", href: "/verticals/engineering-products" },
  ...
  const headingSize = settings?.footer_heading_size ? `${settings.footer_heading_size}px` : "12px";

  const contactAddress = settings?.footer_contact_address || "Besmak Components Private Limited, Plot No. A-45, SIPCOT Industrial Growth Centre, Oragadam, Kanchipuram – 602118, Tamil Nadu, India.";
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0 opacity-[0.12] animate-bg-drift" 
           style={{ 
      ...
      <div className="px-6 max-w-7xl mx-auto relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 space-y-4 flex flex-col items-center text-center">
            <Link href="/" className="inline-block bg-white p-1.5 rounded shadow-sm">
              <Image 
            ...
            <p className="text-white/80 text-sm max-w-xs leading-relaxed">
              Precision engineering and connection systems for global automotive and industrial sectors.
            </p>
            <div className="flex items-center justify-center gap-3 pt-2">
              {socials.map((social: any, idx: number) => {
                ...
                if (!IconComponent || !social.url || social.url.trim() === "" || social.url === "#") {
                  return null;
                }
          ...
          <div className="lg:col-span-2 space-y-3">
            ...
            <ul className="space-y-2 text-[15px] text-white/80">
              {verticals.map((link: any, idx: number) => (
                <li key={idx}>
                  <Link href={link.href} className="hover:text-white/70 transition-colors">{link.title}</Link>
                </li>
              ))}
            </ul>
          </div>
            ...
            <div className="space-y-4 text-[15px] text-white/80">
              <div className="flex gap-2">
                ...
                <span className="leading-relaxed">
                  Besmak Components Private Limited,<br />
                  Plot No. A-45, SIPCOT Industrial Growth Centre,<br />
                  Oragadam, <br />
                  Kanchipuram – 602118,<br />
                  Tamil Nadu,<br />
                  India.
                </span>
              </div>
        ...
        <div className="mt-10 pt-6 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-4 text-[12px] opacity-70">
          <div className="flex flex-wrap justify-center gap-4 sm:gap-8">
            ...
            <div className="flex gap-4">
              ...
              {legal.map((link: any, idx: number) => (
                <Link key={idx} href={link.href} className="hover:text-white transition-colors underline underline-offset-2">
                  {link.title}
                </Link>
              ))}
            </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Link href="https://imaginetventures.com" target="_blank" className="font-semibold hover:text-white transition-colors">
              ImagiNET Ventures
            </Link>
          </div>
        </div>
